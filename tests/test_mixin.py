...
def test_ec3__get_rawdata_non_exist():
    ...
    assert len(ec2_compare.internal.ec2data.get_instances_list()
               ) == len(inst.ec3__get_rawdata('Non-exists'))


def test_ec3__get_rawdata_exists():
    inst = ec2_compare.mixin.AwsInstanceMixin()
    for k in ['m', 'c', 'r']:
        assert len(ec2_compare.data.get(key='InstanceType', value=k)
                   ) == len(inst.ec3__get_rawdata(k))
        assert len(ec2_compare.data.get(key='InstanceType', value=f'{k}5')) < len(
            inst.ec3__get_rawdata(f'{k}5'))
...
def test_ec3__get_instances():
    ...
    for _fixture in zip(
            [
                ...
                {
                    'cpu': 100, 'ram': 786432, 'min_cpu': 8, 'min_ram': 65536,
                    'max_cpu': 36,
                    'InstanceType': ['c5.'],
                    'SupportedArchitectures': ['x86_64'], 'SupportedUsageClasses': 'spot'
                },
        ...
        assert sorted(inst.ec3__get_instances(**_fixture[0])) == sorted(_fixture[1])
...
def test_ec3__get_machines():
    ...
    for _fixture in zip(
        ...
        assert len(inst.ec3__get_machines_for_fleet_request(
            **_fixture[0])) == _fixture[1]
